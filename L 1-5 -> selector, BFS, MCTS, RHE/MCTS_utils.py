...
class Node():
    ...
    def get_explored_MCTS_utils(self):
        return self.state.get_explored_Game_State()
    ...
    def find_children(self):
        ...
        """
        children = set()
        for action in self.state.getLegalPacmanActions(): # this returns a list
            children.add(Node(self.state.generatePacmanSuccessor(action))) # generate succesor returns a state
        return children
    ...
    def find_random_child(self):
        """ Random successor of this board state
            ...
            Returns a Node
        """
        action = choice(self.state.getLegalPacmanActions())# ta metoda wyrzuca nam wskazowki w ktora strone mamy sie udac, po prostu 'akcje', gdzie musimy'iść'
        return Node(self.state.generatePacmanSuccessor(action))# a tutaj ta metoda bierze wskazówki gdzie isc jako parameter i udaje sie do tego node ktorego chcemy zreturnowac
...
def play_game(tree, human=False, deterministic=True):
    ...
    while True:
        if human:  # Human
            ...
            index = -1
            while not valid_move:
                if deterministic:# tutaj chyba po porstu wbojasz ruch wraz z kolejnośćą indexu w array
                    index += 1
                else:
                    index = choice([0,1,2,3,4,5,6,7,8]) # tutaj ranodomowo a jak trafisz na cos co ma juz cos to zaczynasz while loop od poczatku
                if board.tup[index] is not None:
                    pass
                else:
                    valid_move = True
            # print(board.to_pretty_string())
            # no i tutaj 'robosz mow' i zostawiasz albo kolko albo krzyzyk na boardzie
            board = board.make_move(index)
            # print(board.to_pretty_string())
